 ...
 *
 * Revisions:
 * 7  Aug  98   Robert Gasch <rgasch@peoplesoft.com>
 * 	- added typecasts for compile to pass g++ -Wall without warnings
 * 
 ...
 *	- more cleanup
 *	- define BAD_REALLOC to use malloc() instead of realloc()
      ...
      double scaling)
{
   ...
   for (i = 0; i < ndim; i++)
     {
	nSpan = dims [i];
	if (nSpan > (unsigned int)max_factors) max_factors = nSpan; 	// RNG
	if (nSpan > (unsigned int)max_perm) max_perm = nSpan;		// RNG
     }
#else
   ...
   if (scaling && scaling != 1.0)
	...
	for (i = 0; i < (int)nTotal; i += iSign)			// RNG
	  {
	     Re [i] *= scaling;
	     Im [i] *= scaling;
	  }
	  ...
	  int max_perm)
{
   ...
   if (MaxPermAlloced < (unsigned int)max_perm)				// RNG
     {
#ifdef BAD_REALLOC
	if (!MaxPermAlloced)	/* first time */
	  Perm = (int *)malloc (max_perm * sizeof(int));		// RNG
	else
#endif
	  Perm = (int *)realloc (Perm, max_perm * sizeof(int));		//RNG
